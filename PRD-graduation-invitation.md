...
> **Phiên bản:** 2.0
> **Ngày tạo:** 2026-06-09
> **Trạng thái:** Approved for implementation — launch pending event time confirmation + Formspree endpoint setup
...
- Header: 🎓 emoji + "You Are Invited" + "Quách Ngọc Quang" + divider
- Message: lời mời cá nhân
- Details: ngày / địa điểm / sự kiện (3 cột)
...
```javascript
// Endpoint — người dùng tự tạo Formspree form và điền ID
const RSVP_ENDPOINT = 'https://formspree.io/f/FORM_ID';

// POST body gửi đi
{
  name: "Nguyễn Văn Anh",
  attendance: "yes",       // "yes" | "no"
  guestCount: 2,           // 0 nếu attendance = "no"
  message: "Chúc Quang thi tốt!",
  timestamp: "2026-06-09T10:30:00.000Z",
  _gotcha: ""            // anti-spam field (bỏ trống = legit, Formspree tự reject nếu có giá trị)
}
...
4. Copy endpoint ID → paste vào `RSVP_ENDPOINT` trong code
...
- Trước khi submit, kiểm tra localStorage — nếu đã gửi trong 1 phút gần nhất, hiển thị warning
...
|-----------|-------|--------|
| Honeypot field | Thêm input ẩn (`display:none`) tên `_gotcha` — bot tự điền → Formspree reject server-side | Light |
...
| Timeout (>10s) | — | Giữ màn 2, hiện lỗi, cho thử lại |
