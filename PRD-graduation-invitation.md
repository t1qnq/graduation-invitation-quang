...
|-----------|-----------|-------|
| HTML/CSS/JS | Vanilla, 3 file tĩnh (`index.html` + `style.css` + `app.js`) | Không cần build tool, dễ deploy |
...
- 3 file tĩnh (`index.html` + `style.css` + `app.js`) → dễ chỉnh sửa, không cần rebuild, chỉ upload lại
